...
Asia/Seoul timezone. The first implementation focuses on the KOSPI/KOSDAQ
section from the target UI and stores both a latest HTML report and a dated
archive under reports/ so GitHub Pages can publish the generated files.
"""
...
REPORT_ROOT = Path(os.environ.get("MARKET_SUMMARY_REPORT_DIR", ROOT / "reports"))
...
<head>
  ...
  <title>전일 국내 시장 요약 - {{ generated_date }}</title>
  <style>
    :root {
      color-scheme: light;
      --border: #e8e8e8;
...
<body>
  <main>
    ...
    <aside class="notice">
      ...
      코스피·코스닥은 조회 가능한 최근 2개 거래일의 종가 기준으로 계산됩니다.
      <ul>
